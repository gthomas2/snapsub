...
/**
 * Standard library functions for snapsub theme.
 *
...
function theme_snapsub_get_main_scss_content($theme) {
    global $CFG;
    ...
    $pre .= file_get_contents($CFG->dirroot . '/theme/snapsub/scss/pre.scss');
    // Post CSS - this is loaded AFTER the main scss but before the extra scss from the setting.
    $post = file_get_contents($CFG->dirroot . '/theme/snapsub/scss/post.scss');

    // Combine them together.
    $combined = $pre . "\n" . $scss . "\n" . $post;
    file_put_contents('/Users/guy/tmp/web/snapsub.combined.scss', $combined);
    return $combined;
}
 ...
 * It does this by rewriting some settings from being snap specific to snapsub specific.
 * @param $filename
 * @param theme_boost_admin_settingspage_tabs $settings
 * @param string $insertsettings
 */
function theme_snapsub_require_snap_settings($filename, theme_boost_admin_settingspage_tabs $settings,
                                          $insertsettings = null) {
    $str = file_get_contents(__DIR__.'/../../theme/snap/settings/'.$filename);
    $search = <<<'SRCH'
/\$name(?:|\s*)=(?:|\s*)'theme_snap\//
SRCH;
    $replace = '$name = \'theme_snapsub/';
    $str = preg_replace($search, $replace, $str);

    $search = <<<'SRCH'
/admin_settingpage(?:|s*)\(\'themesnap/
SRCH;
    $replace = 'admin_settingpage(\'themesnapsub';
    $str = preg_replace($search, $replace, $str);
 ...
 * @return bool
 */
function theme_snapsub_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
